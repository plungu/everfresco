   ...
   */
   ...
   Alfresco.WikiToolbar.prototype =
   {
      ...
      options:
      {
         ...
         /**
          * The noderef of the current page.
          *
          * @property title
          * @type string
          */
      ...
      onReady: function WikiToolbar_onReady()
      {
         ...
         this.widgets.everfrescoPageButton = Alfresco.util.createYUIButton(this, "everfresco-button", this.onSyncClick,
         {
            disabled: true,
            value: "edit"
         });
         ...
         this.popups.syncDialog = Alfresco.util.createYUIPanel("syncDialog", 
                 {
                    width: "20em",
                    text: '<div class="yui-u"><br />' + this._msg("panel.confirm.sync-msg") + '<br /><br /></div>',
                    buttons: [
                    {
                       text: this._msg("button.sync"),
                       handler:
                       {
                          fn: this.onConfirmSync,
                          scope: this
                       }
                    },
                    {
                 ...
                 this.popups.syncDialog.setHeader(this._msg("panel.confirm.header"));
         ...
         YAHOO.Bubbling.on("syncPage", this.onSyncPage, this);
      },
       ...
       * @param e {object} DomEvent
       */      
      onSyncPage: function WikiToolbar_onSyncPage(e, args)
      {
      ...
      /**
       * Fired when the user confirms that they want to Sync a page. 
       * Kicks off a SYNC request to the Alfresco repo to remove an event.
       *
       * @method onConfirmSync
       * @param e {object} DomEvent
       */
      onConfirmSync: function WikiToolbar_onConfirmSync(e)
      {
         Alfresco.util.Ajax.request(
         {
            ...
            //url: Alfresco.constants.PROXY_URI + "slingshot/wiki/page/" + this.options.siteId + "/" + encodeURIComponent(this.options.title) + "?page=wiki",
            url: Alfresco.constants.PROXY_URI + "everfresco/applyEverfresco.json?title=" + encodeURIComponent(this.options.title) + "&siteName=" + this.options.siteId,
            successCallback:
            {
               fn: this.onPageSync,
               scope: this
       ...
       * @param e {object} DomEvent
       */
      onPageSync: function WikiToolbar_onPageSync(e)
      {
         // Redirect to the wiki landing page
         var landingPage = Alfresco.constants.URL_PAGECONTEXT + "site/" + this.options.siteId + "/wiki";
         if (window.location.pathname == landingPage)
       ...
       * Pops up the rename dialog.
       *
       * @method onEFSyncClick
       * @param e {object} DomEvent
       */
      onEFSyncClick: function WikiToolbar_onEFSyncClick(e)
      {
    	  alert("SYNC");
    	  ...
    	  Alfresco.util.Ajax.request(
	         {
	            method: Alfresco.util.Ajax.POST,
	            url: Alfresco.constants.PROXY_URI + "everfresco/applyEverfresco.json?nodeRef={nodeRef}",
...
//          });
       ...
       * @param e {object} DomEvent
       */
      onSyncClick: function WikiToolbar_onSyncClick(e)
      {
         this.popups.syncDialog.show();
      },
