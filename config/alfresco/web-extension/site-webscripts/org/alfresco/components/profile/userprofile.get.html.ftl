<script type="text/javascript">//<![CDATA[
   var userProfile = new Everfresco.UserProfile("${args.htmlid}").setOptions(
   {
      userId: "${user.name}",
      profile: {
         ...
         efUserName: "<#if profile.properties.efUserName??>${profile.properties.efUserName?js_string}</#if>",
         efPassword: "<#if profile.properties.efPassword??>${profile.properties.efPassword?js_string}</#if>"
      }
   }).setMessages(
      ${messages}
   );
...
<div id="${el}-body" class="profile">
   <div id="${el}-readview" class="hidden">
      ...
      <div class="viewcolumn">
         ...
         <div class="header-bar">${msg("label.ef.info")}</div>
         <#if profile.properties.efUserName?? && profile.properties.efUserName?length!=0>
         <div class="row">
            <span class="fieldlabelright">${msg("label.ef.username")}:</span>
            <span class="fieldvalue">${profile.properties.efUserName?html}</span>
         </div>
         </#if>
         <#if profile.properties.efPassword?? && profile.properties.efPassword?length!=0>
         <div class="row">
            <span class="fieldlabelright">${msg("label.ef.password")}:</span>
            <span class="fieldvalue">${profile.properties.efPassword?html}</span>
         </div>
         </#if>
      </div>
   </div>
   ...
   <div id="${el}-editview" class="hidden">
      ...
      <div class="header-bar">${msg("label.ef.info")}</div>
      <div class="row">
         <span class="label"><label for="${el}-input-efUserName">${msg("label.ef.username")}:</label></span>
         <span class="input"><input type="text" maxlength="256" size="30" id="${el}-input-efUserName" value="" <@immutablefield field="efUserName" /> /></span>      
      </div>
      <div class="row">
         <span class="label"><label for="${el}-input-efPassword">${msg("label.ef.password")}:</label></span>
         <span class="input"><input type="text" maxlength="256" size="30" id="${el}-input-efPassword" value="" <@immutablefield field="efPassword" /> /></span>      
      </div>
